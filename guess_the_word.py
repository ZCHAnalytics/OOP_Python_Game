...
class Hangman: 
    ...
    def check_guess(self, guess) -> None: 
        ...
        if guess in self.word:
            ...
            # Update the list of dashes representing a chosen word replacing dashs with a correctly guessed letter.
    ...
    def ask_for_input(self):
        """ 
        Asks for player's input, checks if it is a single, alphabetical character (not a number) and that it was not used before.  
        """
        while True:
            guess = input('Please, enter a letter: ') # Asks the player to input a letter.
            if not (guess.isalpha() and len(guess) == 1): # Checks if the player's input is single alphabetical (not a number) character.
                print('\nInvalid letter. Please, enter a single alphabetical character.')
            elif guess in self.list_of_guesses: # Checks if the player's input has previously been used. 
                print('\nYou already tried that letter!')
                print(f'\n{self.word_guessed}')
            else:
                self.list_of_guesses.append(guess) # Updates the list of unique letters inputted by the player.
                self.check_guess(guess) # Checks if the inputted letter is in the chosen word.
                print('\nCurrent word: ', self.word_guessed) # Print the updated list of dashes for the player to see game progress. 
                break
...
def play_game(word_list):
    ...
    while True:
        if game.num_lives == 0: # Once the number of lives reaches zero, print messages that the user lost the game.   
            print(f'\nYou lost! the word was {game.word}')
            break
        elif game.num_letters > 0: # Continues the game if a life is still available.
            game.ask_for_input()
        else: # Print message to congratulate the player with winning the game. 
